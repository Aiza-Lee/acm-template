#include "aizalib.h"
/*
 * Universal Euclidean Algorithm (万能欧几里得算法)
 *
 * Overview:
 *     在网格直线上行走的抽象代数框架，支持任意满足结合律的幺半群（Monoid）状态合并。
 *     给定直线 y = (a*x + b) / c，沿 x in [1, n] 追踪穿过网格线的操作序列（U 向上、
 *     R 向右），以类似辗转相除法在 O(log(min(a, c, n)))
 *     步内计算操作序列在代数结构下的连乘积。
 *
 * API:
 *     solve(n, a, b, c, u, r)       — 求解 x \in [1, n] 核心操作序列积,复杂度
 *                                      O(log(min(a, c)) * T_mul)
 *     solve_full(n, a, b, c, u, r)  — 求解包含 x=0 处 U^(b/c) 前缀的完整路径积,
 *                                      复杂度 O(log(min(a, c)) * T_mul)
 *     query_like_euclid(n, a, b, c) — 便捷计算 P5170 类欧三和(f, g, h),
 *                                      支持整型或模数类 T
 *     struct LikeEuclidNode         — 泛型类欧三和状态节点,维护 count、一次和、
 *                                      平方和及乘积交叉项
 *     struct LikeEuclidResult       — 泛型类欧三和返回值聚合 (f=\sum y, g=\sum i*y,
 *                                      h=\sum y^2)
 *
 * Notes:
 *     1. 参数要求: a >= 0, b >= 0, c > 0, n >= 0。内部坐标运算使用 i128 防御溢出。
 *     2. 概念约束: 自定义 Node 需支持默认构造单位元与乘法结合律 operator*。
 *     3. 统计范围: solve() 统计 x in [1, n] 区间贡献；若需包含 i=0 处使用
 *        solve_full()。
 *     4. 模数兼容: LikeEuclidNode<T> 默认 T=i64，传入模数类时自动按模运算合并。
 *
 * Related:
...
struct UniversalEuclidean {
    ...
    static Node solve(i64 n, i64 a, i64 b, i64 c, Node u, Node r) {
        ...
        return qpow(r, cnt_p) * u *
               solve(m - 1, c, (c - b - 1) % a, a, r, u) * qpow(r, cnt_s);
    }
