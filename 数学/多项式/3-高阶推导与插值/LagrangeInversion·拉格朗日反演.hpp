 ...
 *
 * Overview:
 *     给定多项式 F(x) 满足 F(0) = 0, F'(0) != 0，求其复合逆 G(x) (满足 G(F(x)) = x)
 *     的第 n 项系数 [x^n] G(x)，以及扩展拉格朗日反演 [x^n] H(G(x))。
 *
 * API:
 *     coeff(Poly F, int n)                     — 计算复合逆 G(x) 的第 n 项系数
 *                                                 [x^n] G(x)
 *     generalized_coeff(Poly F, Poly H, int n) — 计算复合函数 H(G(x)) 的第 n
 *                                                 项系数 [x^n] H(G(x))
 *
 * Notes:
