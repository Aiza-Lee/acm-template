...
/*
 * CongruenceShortestPath·同余最短路
 *
 * Overview:
 *     跑 Dijkstra 得到每个余数下可达的最小值。
 *     用于解决 Frobenius 硬币问题等——判断一个数能否表示为给定正整数的非负整数线性
 *     组合。
 *
 * API:
 *     solve(base, values)      — 以 base 为模数，values 为其余系数。返回
 *                                 std::vector<i64> dist, 其中 dist[r] 为模 base 余
 *                                 r 的最小可达值。dist[0] = 0。不可达的余数设为
 *                                 INF。
 *     reachable(dist, target)  — 判断 target 是否可表示为给定系数的非负整数组合。
 *     max_not_achievable(dist) — 返回最大不可表示数。要求所有余数均可达（即 gcd =
 *                                 1），否则返回 -1。
 *
 * Notes:
 ...
 *     4. 若 gcd(base, values...) = 1 则所有余数可达，最大不可表示数为 max(dist) -
 *        base。
 *     5. 常见变种：若集合包含 base 本身（base = min(a_i)），values 中不应再包含
 *        base。
 */
struct CongruenceSP {
