...
/*
 * KruskalRebuildTree·Kruskal重构树
 *
 * Overview:
 *     Kruskal 重构树在 Kruskal 最小生成树算法的基础上构建重构树。
 ...
 *
 * API:
 *     Result  — 包含 n, root, parent, weight 的结果结构体。
 *     solve() — 构建重构树，返回结果结构体。
 *
 * Notes:
 *     模板参数: T: 边权类型, 默认为 i64
 ...
 *     4. 用法/技巧: 4.1 连通时 root 数组只有一个元素；
 *        不连通时每个连通分量对应一个根。4.2 配合 LCA 模板使用：两点 LCA
 *        的权值即为它们路径上最大边权的最小值。4.3 solve() 每次会重置并查集，
 *        因此同一对象可重复求解当前边集。
 */
...
struct KruskalReconstructionTree {
    ...
    KruskalReconstructionTree(int n) : graph(n), fa(n + 1), _n(n) {
