 ...
 *
 * Overview:
 *     Prim 一种求解加权无向图最小生成树的算法。
 *     从任意一个顶点开始，每次选择距离当前生成树最近的节点加入生成树，
 *     直到所有节点都被加入。
 *
 * API:
 ...
 *     4. 用法/技巧: 4.1 使用 std::priority_queue 懒删除，适合显式稀疏图；
 *        稠密图可改邻接矩阵 O(V^2)。4.2 solve(s) 每次会重置 dis/vis，
 *        同一对象可从任意起点重复求当前图 MST。
 */
...
struct Prim {
    ...
    Prim(int n)
        : graph(n), dis(n + 1, std::numeric_limits<T>::max()),
          vis(n + 1, 0) {}
