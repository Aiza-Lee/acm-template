 ...
 *     int count_in_arborescence(int root) const         — 返回根为 root、
 *                                                          边指向根的树形图个数
 *     int count_out_arborescence(int root) const        — 返回根为 root、
 *                                                          边从根向外的树形图个数
 *
 * Notes:
 *     模板参数: 无
 ...
 *     4. 用法/技巧: 4.1 支持重边，w 表示边数/权值 multiplicity。4.2
 *        有向情形里自环不会出现在树形图中，因此会被自动忽略。
 */
struct MatrixTree {
    int n;                                          // 点数
    ...
    MatrixTree(int n)
        : n(n), und_lap(n + 1, std::vector<int>(n + 1)),
          dir(n + 1, std::vector<int>(n + 1)) {}
...
private:
    ...
    static std::vector<std::vector<int>> _build_in_laplacian(
    ...
    static int _count_from_laplacian(
        const std::vector<std::vector<int>>& lap, int root) {
