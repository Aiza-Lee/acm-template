 ...
 *
 * Overview:
 *     用单调栈在线性时间内构造二叉树，使其中序遍历等于原序列，堆序由比较器决定。
 *
 * API:
 *     CartesianTree()                   — 默认构造空树
 *     CartesianTree(a, cmp = Compare()) — 用 1-based 序列 a 构造笛卡尔树
 *     rebuild(a, cmp = Compare())       — 用新序列重构笛卡尔树
 *
 * Notes:
 *     1. 输入序列需为 1-based，a[0] 仅作占位；支持 n=0（空序列，构造后 root=0）。
 *     2. Time: 构造 O(N)；Space: O(N)。
 *     3. 中序遍历一定还原原下标顺序；父子关系满足比较器定义的堆序。
 *     4. 成员 root/fa/ls/rs 可直接使用；相等元素按下标稳定，较小下标更靠上。
 */
