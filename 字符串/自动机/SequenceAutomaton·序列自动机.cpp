 ...
 *      match(t)            // 判断 t 是否为 s 的子序列
 *      next_pos(u, c)      // 在 s 的第 u 个位置之后，最早出现的字符 c 的位置
 *
 * Note:
 ...
 *      3. 状态空间 {0, 1, ..., n+1}:
 *          - 状态 0 = 初始状态（尚未匹配任何字符）
 *          - 状态 u ∈ [1, n] = 当前匹配的前缀在原串中结束于第 u 个字符（1-based 位置）
 *          - 状态 n+1 = 不存在（无更多匹配）
 *      4. next_pos(u, c) 中 u 的合法范围是 [0, n]；u = n+1 或更大时直接返回 n+1
 */
struct SequenceAM {
    int n;
