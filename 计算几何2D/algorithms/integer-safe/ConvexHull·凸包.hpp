 ...
 *
 * API:
 *     convex_hull(pts)           — 点集凸包；结果按逆时针排列，
 *                                   自动去重且不包含多余共线点。TimeO(N log N),
 *                                   Space O(N)。
 *     convex_hull_centroid(hull) — 凸包（逆时针）几何重心；空凸包返回(0, 0)。Time
 *                                   O(N)。
 *
 * Notes:
 *  模板参数 T: 点集 / 凸包顶点坐标类型。
 *  若需保留共线点，应将 cross 判断改为 < 0（而非 <= 0）。
 *  convex_hull_centroid 返回 Point<ld>，与输入 T 无关；底层转调 polygon_centroid；
 *  area=0 时由其兜底返回首顶点。
 *
 * Related:
...
Polygon<T> convex_hull(std::vector<Point<T>> pts) {
    ...
    rep(i, 0, n - 1) {
        while (k > 1 &&
               sgn((hull[k - 1] - hull[k - 2]).cross(pts[i] - hull[k - 1])) <= 0) {
            --k;
        }
        hull[k++] = pts[i];
    }
