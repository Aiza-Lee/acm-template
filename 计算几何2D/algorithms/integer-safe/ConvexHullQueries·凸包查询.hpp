 ...
 *
 * API:
 *     argmax_dot(poly, dir)       — 沿 dir 点积最大的顶点索引。O(log N)。
 *     argmin_dot(poly, dir)       — 沿 dir 点积最小的顶点索引。 O(log N)。
 *     tangent_from_point(poly, p) — 左 / 右切线端点索引。O(N)。
 *     point_in_convex(poly, p)    — 严格内部 1，边界 0，外部 -1。O(log N)。
 *
 * Notes:
 *  模板参数 T: 坐标类型。
 *  输入必须是逆时针凸多边形；n<3 时调用行为见各函数实现。
 *  argmax / argmin 把点积沿顶点视为循环单峰函数用三分搜索；dir
 *  与某边平行时取该边起点索引。
 *  tangent_from_point 通过叉积判定切点，p 在凸包内或上时返回包含 p 的对边；该函数为
 *  O(N)，替换为 argmax_dot / argmin_dot 的 O(log N) 方案在退化情形（如 p
 *  在某轴延长线上）会出现 ties 导致选错顶点。
 */
...
int argmax_dot(const Polygon<T>& poly, const Point<T>& dir) {
    ...
    return _chq_detail::_cyclic_ternary<T>(
        n, [&](int i) { return poly[i % n].dot(dir); }
    );
}
...
// first = 左切线（多边形在 p→poly[first] 右侧），second = 右切线（多边形在
// p→poly[second] 左侧）
// p 应为外部点；若是内部点则返回的切线退化为包含 p 的对边
// note: O(N) 而非 O(log N) — 用 dot-product argmax/argmin 的 O(log N)
// 替代方案在某些
...
std::pair<int, int> tangent_from_point(const Polygon<T>& poly, const Point<T>& p) {
    ...
    for (int i = 0; i < n; ++i) {
        ...
        // 边 (k, i) 方向: p 在其右侧（即 cross(poly[i]-poly[k], p-poly[k]) > 0，
        // 注意方向反转）
        int c1 = sgn((poly[j] - poly[i]).cross(p - poly[i])); // p 关于 (i, j) 符号
        int c2 = sgn((poly[i] - poly[k]).cross(p - poly[k])); // p 关于 (k, i) 符号
...
int point_in_convex(const Polygon<T>& poly, const Point<T>& p) {
    ...
    auto on_segment = [&](const Point<T>& a, const Point<T>& b) {
        return cmp(p.x, std::min(a.x, b.x)) >= 0 &&
               cmp(p.x, std::max(a.x, b.x)) <= 0 &&
               cmp(p.y, std::min(a.y, b.y)) >= 0 &&
               cmp(p.y, std::max(a.y, b.y)) <= 0;
    };
    ...
    bool on_boundary = (t2 == 0) || (t1 == 0 && lo == 1) ||
                       (t3 == 0 && lo + 1 == n - 1);
