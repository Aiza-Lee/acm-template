 ...
 *
 * API:
 *     half_plane_intersection(lines) — 半平面交点集（按出现顺序）。Time O(N log N),
 *                                       Space O(N)。空集返回{}；无界交集自动追加包围
 *                                       盒裁剪为有限多边形后返回（见 Notes）。
 *
 * Notes:
 ...
 *  排序键按方向 (b, -a) 的极角升序；方向相同的平行直线仅保留法向量侧更"靠内"的一条
 *  （即 c 更小、保留区域更小者），以避免主循环冗余弹队。
 *  反向平行线对（两个方向相反的半平面）经 safe_intersect 兜底返回空集。
 *  增量算法要求交集有界：当去重后的方向角出现 ≥ π 的空隙时法向量不张成整个平面，
 *  交集无界（可能为空）。
 *  此时自动追加一个大小为输入截距量级 1e6 倍的包围盒，把结果裁剪成有限凸多边形；
 *  判定无界可比较返回顶点是否落在包围盒边界上，或按需自行处理。空集与无界互斥，
 *  返回非空即交集非空。
 *
 * Related:
...
std::vector<Point<T>> half_plane_intersection(std::vector<Line<T>> lines) {
    ...
    // 无界判定：相邻方向角（含首尾环绕）最大空隙 ≥ π => 法向量被某个半圆包住 =>
    // 交集无界。
    ...
    // 无界时追加包围盒强制有界；包围盒大小取输入直线截距量级的 1e6 倍，
    // 保证只裁剪真正的"无穷远"部分。
    if (unbounded) {
        T maxi = 1;
        for (const auto& l : ls) {
            T cand = std::abs(l.a) > std::abs(l.b) ?
                     std::abs(l.c / l.a) : std::abs(l.c / l.b);
