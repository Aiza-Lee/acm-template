 ...
 *
 * API:
 *     distance_to_segment(p, s)          — 点到线段的有符号最短距离。O(1)。
 *     segment_intersection_point(s1, s2) — 两线段交点，要求二者确实相交。O(1)。
 *     segment_segment_distance(s1, s2)   — 两线段间的最短距离。O(1)。
 *     SegmentFP                          — Segment<ld> 的常用别名。
 *
 * Notes:
 *  仅支持浮点坐标；调用 segment_intersection_point 前应先用 segment_intersect
 *  确认相交。
