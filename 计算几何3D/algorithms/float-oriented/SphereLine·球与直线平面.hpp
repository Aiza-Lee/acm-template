 ...
 *
 * API:
 *     sphere_line_intersection(line, sphere) — 球与直线交点;按 t 升序返回0/1/2
 *                                               个交点。O(1)。
 *     two_planes_intersection(p1, p2)        — 两平面的交线;法向量平行或重合时返回
 *                                               nullopt。O(1)。
 *     three_planes_intersection(p1, p2, p3)  — 三平面唯一点;Crammer行列式为 0返回
 *                                               nullopt。O(1)。
 *
 * Notes:
 *  球-直线的求交:代入 line.p0 + t·dir,展开 |P-c|²=r² 为 |d|²·t² + 2 d·(p0-c) t +
 *  |p0-c|²-r² = 0。
 *  两平面交线:方向 = n1 × n2 (n1, n2 为两法向量,需非零);取一个 2×2 minor 不为零,
 *  用 Crammer 子式求出 z 固定的 (x, y)。
 *  三平面交点:对 3×3 系数矩阵用全 Crammer 法则;若 |n1, n2, n3| = 0 (三平面共轴)
 *  返回 nullopt。
 *
 * Related:
 *  Sphere·球.hpp::sphere_plane_intersection / sphere_sphere_intersection:
 *  同段其他求交入口。
...
std::vector<Point<T>> sphere_line_intersection(
    const Line<T>& l, const Sphere<T>& s) {
    Point<T> oc = l.p0 - s.c;
    T a = l.dir.dot(l.dir);
...
template<typename T>
requires std::is_floating_point_v<T>
std::optional<Line<T>> two_planes_intersection(
    const Plane<T>& p1, const Plane<T>& p2) {
    Point<T> n1(p1.a, p1.b, p1.c);
    Point<T> n2(p2.a, p2.b, p2.c);
    Point<T> dir = n1.cross(n2);
    // n1 // n2 -> 平行 / 重合
    if (is_zero(dir.len2())) return std::nullopt;

    // 在 n1 × n2 ≠ 0 时, [n1; n2] 至少存在一个 2×2 子行列式非零:
    // 候选 minor_xy / minor_yz / minor_zx。
    // 任选一个非零的,固定被去掉的坐标 (比如 xy 非零时固定 z = 0),解 n1·P = -d1,
    // n2·P = -d2 的 2x2 子方程
...
template<typename T>
requires std::is_floating_point_v<T>
std::optional<Point<T>> three_planes_intersection(
