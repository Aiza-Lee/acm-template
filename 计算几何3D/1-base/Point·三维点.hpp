 ...
 *     triple(p, q)       — 三重标量积 (this × p) · q; 用于共面判定 / 有向体积。
 *                           O(1)。
 *
 * Notes:
 *  浮点比较经 cmp / sgn 使用 EPS。
 *  整数坐标下,叉积与标量三重积需调用方自行控制 T 溢出（|x|,|y|,|z| ≤ 1e6 时 i64
 *  安全）。
 *
 * Related:
...
struct Point {
    ...
    Point operator*(T k) const { return Point(x * k, y * k, z * k); }
    Point operator/(T k) const { return Point(x / k, y / k, z / k); }

    // 字典序 (先 x 后 y 后 z), 经 cmp 兼容整数严格 / 浮点 EPS
    ...
    T dot(const Point& p)   const { return x * p.x + y * p.y + z * p.z; }
    Point cross(const Point& p) const { // 叉积 p x q (向量)
        return Point(
            y * p.z - z * p.y,
            z * p.x - x * p.z,
            x * p.y - y * p.x
    ...
    T triple(const Point& p, const Point& q) const { // (this x p) · q
