 ...
 *     eval(p)                               — 代入 ax + by + cz + d。O(1)。
 *     side(p)                               — 返回 +1 / 0 / -1（正 / 平面 / 负），
 *                                              使用 sgn。O(1)。
 *     normal()                              — 法向量 (a, b, c)；浮点要求归一化请在
 *                                              外部用 normalize。
 *     distance_to_point(plane, p)           — 有符号距离（正值在法向量一侧）。
 *                                              浮点。O(1)。
 *     projection(plane, p)                  — 点在平面上的正交投影。浮点。O(1)。
 *     reflection(plane, p)                  — 点关于平面的对称点。浮点。O(1)。
