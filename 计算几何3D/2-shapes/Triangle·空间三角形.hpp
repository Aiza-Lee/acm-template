 ...
 *
 * API:
 *     triangle_squared_area_2(a, b, c) — 4 倍面积平方 = |(b - a) ×(c - a)|²;
 *                                         整数安全(T范围内)。O(1)。
 *     triangle_area(a, b, c)           — 三角形面积。浮点。O(1)。
 *     triangle_normal(a, b, c)         — 单位法向量;退化时 (叉积零)返回零向量。
 *                                         浮点。O(1)。
 *     signed_solid_angle_2(a, b, c)    — 两倍"有向"立体角(|(b-a)·(c-a) |
 *                                         等投影量)不存在;保留占位。
 *     point_in_triangle(p, a, b, c)    — +1 (内) / -1 (外) / 0(边上或退化)。浮点。
 *                                         O(1)。
 *     tetrahedron_volume(a, b, c, d)   — 浮点无符号体积。O(1)。
 *
 *     signed_volume_tetrahedron(a, b, c, d) — 四面体 6 倍有向体积 = ((b -a) ×
 *                                              (c - a)) · (d - a);整数坐标受限时整
 *                                              数安全。O(1)。
 *
 *     barycentric(p, a, b, c, &alpha, &beta, &gamma) — 重心坐标,三参数 α,β,γ 满足α
 *                                                       + β + γ = 1。浮点。
 *
 * Notes:
 *  面积 / 体积函数面对共线 / 共面退化返回 0,几何意义由调用方把握。
 *  barycentric 输出三参数 α, β, γ;α 对应顶点 a。点位于三角形边时 (α,β,γ 中任一为 0)
 *  point_in_triangle 返回 0。
 *  signed_volume_tetrahedron 的 6 倍有符号版本便于判内 / 外;几何体积 = |signed| /
 *  6。
 *
 * Related:
...
template<typename T>
requires std::is_floating_point_v<T>
void barycentric(Point<T> p, Point<T> a, Point<T> b, Point<T> c,
...
// 点是否在三角形内 (含边):先验证 P 在三角形平面上 (signed_volume_x6 ≈ 0), 然后用 2D
// 重心坐标判定 (在平面上时 v0, v1, v2 三者线性相关,β+γ = 1 - α)。
template<typename T>
requires std::is_floating_point_v<T>
