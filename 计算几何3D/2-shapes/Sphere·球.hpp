 ...
 *
 * API:
 *     Sphere<T>(c, r)                    — 由圆心 c 与半径 r构造; 要求浮点类型。
 *     contains(s, p)                     — 点 p 是否在球内(含边界)。O(1)。
 *     on_sphere(s, p)                    — 点 p 是否在球面上(浮点容差)。O(1)。
 *     Circle<T>(c, r)                    — 球求交返回的圆类型;r< 0 表示无效/无交。
 *     sphere_plane_intersection(s, pl)   — 球与平面交圆;球心到平面 |d| > r 时r <
 *                                           0。O(1)。
 *     sphere_sphere_intersection(s1, s2) — 球与球交圆;返回Circle.r 的判定规则:r =
 *                                           0 外切, r > 0相交, r < 0 包含
 *                                           /无意义(同心不等径r=-1; 同心等径 r=-1)。
 *     tangent_points_to_sphere(p, s)     — 从点 p 到球的外切点（两点中较近的一个）;
 *                                           点位于球内或球面上返回nullopt。O(1)。
 *     SphereFP                           — Sphere<ld> 的常用别名。
 *
 * Notes:
 *  sphere_sphere_intersection 返回的圆心位于两球心连线上,半径 = sqrt(r1² - a²),a =
 *  (r1² - r2² + d²) / (2d)。
 *  tangent_points_to_sphere 仅返回一个切点(较近 P 的那条);另一条需绕 PC 轴旋转
 *  180°(当前接口不重复提供)。
 *
 * Related:
...
Circle<T> sphere_sphere_intersection(const Sphere<T>& s1, const Sphere<T>& s2) {
    ...
    T d2 = dd.len2();
    if (is_zero(d2)) {                                            // 同心
        return Circle<T>(s1.c, -1); // 同心无交/等径重合统一 r = -1
    }
    T d = std::sqrt(d2);
    // 交圆心在 d 方向距 s1.c 的距离
    T a = (s1.r * s1.r - s2.r * s2.r + d2) / (2 * d);
    T h2 = s1.r * s1.r - a * a;
